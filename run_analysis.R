...
ind <- grep("(mean|std)\\(\\)", f)  # we are interested in columns with
                                    # "mean()" or "std()" in their names
names(Dat) <- sapply(f, # make.names {base} can do it, but this is costumized
                     function(s){
                         s <- gsub("\\(\\)", "", s)
                         s <- gsub("-", "_", s)
                         s <- gsub(",", ".", s)
...
for(i in 1:length(Ds1)){
    ...
    for(j in 1:30){
        m <- NULL
        for(k in 3:ncol(d))
            m <- c(m, mean(d[d[2] == j, k], na.rm = TRUE))
        Res <- rbind(Res, m)
    }
}
colnames(Res) <- colnames(D1)[3:ncol(D1)]
rownames(Res) <- NULL
#
